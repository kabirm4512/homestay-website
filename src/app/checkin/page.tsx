...
function CheckinContent() {
  ...
  useEffect(() => {
    if (bookingParam && bookings.length > 0) {
      const found = bookings.find(
        (b) =>
          b.id === bookingParam ||
          b.bookingReference.toLowerCase() === bookingParam.toLowerCase() ||
          b.guest.phone.replace(/[^0-9]/g, '') === bookingParam.replace(/[^0-9]/g, '')
      );

      if (found) {
        loadBookingIntoForm(found);
      } else {
        // Check localStorage directly in case context is still loading
        try {
          const raw = localStorage.getItem('wp_crm_bookings');
          if (raw) {
            const parsed: CRMBooking[] = JSON.parse(raw);
            const rawFound = parsed.find(
              (b) =>
                b.id === bookingParam ||
                b.bookingReference.toLowerCase() === bookingParam.toLowerCase()
            );
  ...
  const handleManualSearch = (e: React.FormEvent) => {
    ...
    const cleanQueryPhone = bookingQuery.replace(/[^0-9]/g, '');

    const found = bookings.find(
      (b) =>
        b.bookingReference.toLowerCase() === query ||
        b.id.toLowerCase() === query ||
        (cleanQueryPhone && b.guest.phone.replace(/[^0-9]/g, '').includes(cleanQueryPhone)) ||
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Also directly ensure localStorage sync for cross-tab persistence
    try {
      const raw = localStorage.getItem('wp_crm_bookings');
      if (raw) {
        const parsed: CRMBooking[] = JSON.parse(raw);
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-sand-200 text-center space-y-4">
              ...
              <form onSubmit={handleManualSearch} className="max-w-md mx-auto space-y-3 pt-2">
                ...
                <button
                  type="submit"
                  className="w-full py-3 bg-forest-900 hover:bg-forest-800 active:scale-98 text-white rounded-2xl font-bold text-sm shadow-md transition-all cursor-pointer"
                >
                  Find My Reservation
                </button>
              </form>
              ...
              {notFound && (
                <div className="p-3 bg-rose-50 border border-rose-200 rounded-2xl text-xs text-rose-800 flex items-center justify-center space-x-2 max-w-md mx-auto animate-fade-in">
                  <AlertCircle className="w-4 h-4 shrink-0" />
                  <span>Reservation not found. Please verify your reference or call reception.</span>
                </div>
              )}
            </div>
